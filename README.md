...
# 3. ChromaDB 구축 (Steam 리뷰 데이터 필요)
python static_rag/build_chroma_db.py

# 4. 시뮬레이션 실행
python static_zero_shot/simulation_model_a.py   # Exp 1: Zero-Shot
python static_rag/simulation_model_b.py         # Exp 2: Static RAG
python time_aware_rag/simulation_model_c.py     # Exp 3: Time-Aware RAG

# 5. 상관관계 평가
python evaluate_correlation.py --model_csv time_aware_rag/Team3_TimeAware_Results_Final.csv \
                                --model_name Team3_TimeAware --type dynamic
```
...
| 담당 | 내용 |
|------|------|
| **나 (석호)** | Static RAG 설계·구현(`static_rag/`), Time-Aware RAG 설계·구현(`time_aware_rag/`), 페르소나 생성 시스템 구현(`utils/persona_generator.py`), 쿼리 전략 설계(`utils/search_queries.py`), 공통 LLM 설정 구현(`utils/llm_config.py`) |
...
GAMER_TYPE_QUERIES = {
    "ultimate_gamer":  [...],  # 그래픽·성능·몰입감 중심 10개
    "cloud_gamer":     [...],  # 최적화·저사양 중심 10개
...
**"왜 더 비싼 임베딩 모델을 써야 하는가?"**에 대한 핵심 근거입니다.
...
2. **주가 음의 상관(-0.578)이 무결성 증명**: 에이전트들이 주가 폭락에 흔들리지 않고 Steam 여론 회복에 맞춰 구매 의사를 높였습니다. 만약 주가와 양의 상관이 나왔다면, 에이전트가 게임의 본질적 가치가 아닌 외부 소음(Speculation)에 휘둘렸다는 뜻이 됩니다. 에이전트들은 **'진짜 게이머'처럼 행동**했습니다.
